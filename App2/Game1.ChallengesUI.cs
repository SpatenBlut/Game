...
public partial class Game1
{
    void DrawChallenges()
    {
        ...
        for (int i = 0; i < CHALLENGES.Length; i++)
        {
            ...
            TxtMed(ch.Title, startX + 44, ty + 8, titleCol);
            TxtMed(ch.Desc,  startX + 44, ty + 8 + _fontMed.LineSpacing + 2, new Color(100, 110, 150));
            ...
            int claimMY = ty + tileH/2 - _fontMed.LineSpacing/2;
            if (complete)
            {
                ...
                TxtMed("CLAIM", claimX + claimW/2 - TxtMedW("CLAIM")/2, claimMY, new Color(150, 220, 150));
            }
            else
            {
                string rewardStr = $"+{ch.Coins}";
                TxtMed(rewardStr, claimX + claimW/2 - TxtMedW(rewardStr)/2, claimMY, new Color(70, 80, 55));
            }
            visIdx++;
        }
